	...
	for i in range( 0, len( lines )  ):
		...
		if isStepLine( lines[i] ):
			...
			for j in range( 0, len( list_of_row_numbers_we_want )  ):
				...
				list_of_atoms_of_interest.append( lines[i + int(list_of_row_numbers_we_want[j])] )
...
def getListOfXYZValues( chosen_lines_of_interest ):
...
def getListOfAngles( xyz_values ):
		...
		while i < len( xyz_values ):
			...
			#print str( distance_AC )
			#time.sleep(10)
			angle = calculateAngle( distance_BA, distance_BC, distance_AC )
			#print str( angle )
			#time.sleep(10)
...
def addAnglesToStepLines( wanted_steps, angles ):
	...
	for i in range( 0, len( wanted_steps ) ):
		...
		if isStepLine( wanted_steps[i] ):
			...
			wanted_steps[i] = wanted_steps[i] + " Angle: " + str( angles[next_index_of_angles_to_add] )
...
print ""
print "           The entered path to file is:   " + path
print "               The desired distance is:   " + desired_distance
...
#print str( number_of_atoms )
#print str( string_to_write )

writesToFile( chosen_prefix, path, string_to_write )

print "Examine the file with the desired steps and choose the \n3 atoms that will be a part of the angle:"


list_of_row_numbers_we_want = [] 
list_of_row_numbers_we_want = inputWantedRows()
...
xyz_values = getListOfXYZValues( chosen_lines_of_interest  ) 
...
#print str( lines_of_interest)
#s = getInfoStringToWriteToFile( addNumberOfAtoms( lines_of_interest, number_of_atoms ) ) 

lines_with_atom_number = addNumberOfAtoms( lines_of_interest, number_of_atoms ) 


writesToFile( chosen_prefix, path, listToString( lines_with_atom_number ) ) 
#writesToFile( "test_", path, listToString( lines_with_atom_number ) ) 
